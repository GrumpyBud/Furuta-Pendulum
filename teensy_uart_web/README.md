...
- **Pendulum speed during swing-up:** high bottom speed is required to reach upright, so `SWING_UP` does not apply the setup/balance `50 rad/s` filtered-rate fault. Its energy controller removes excess energy while the wider angle-dependent raw-step validator, motor torque/current limits, ODrive velocity limiting, predictive arm travel, arm overspeed, dead-man, and watchdog remain active. The strict filtered-rate limit resumes outside swing-up. Actual SPI/parity failures always remain a separate three-strike encoder fault that invalidates zero.
