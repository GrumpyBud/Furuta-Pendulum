...
10. After bounded upright trials are consistently stable, use **Run** for guarded swing-up tuning. Let go with the arm stopped and the pendulum generally downward, then hold Space. The controller centers the arm with a `0.45 rad/s` speed ceiling and up to `0.300 N m` of breakaway torque, holds it while the pendulum settles, and then starts the 10 A-clamped, 20-second energy trial. The web controls expose strength through `20.0`, damping and centering through `1.0`, startup nudge through `0.300 N m`, and a direct maximum-swing-torque setting through `0.300 N m`; the `0.3063 N m` firmware hard clamp cannot be bypassed. Invalid input identifies the exact field instead of producing a generic rejection. The trial attempts the upright catch with the validated balance gains and logs all preparation/control modes and five swing settings in every CSV. Full automatic motion remains locked until these trials are reviewed.
