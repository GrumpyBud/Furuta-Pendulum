...
- **UART faults during motion:** shorten/separate UART wiring, verify the ODrive is not emitting unrelated debug text, and inspect `loop_us`/`max_loop_us` in the downloaded CSV. Filtered position is sent once and retained inside ODrive; a short `u 0` feeds its watchdog without crowding the next feedback request. During ODrive-managed centering, arm feedback is requested at 100 Hz with a 9 ms reply window; three consecutive misses or 35 ms without valid feedback stops the trial before the 50 ms ODrive watchdog expires. The 200 Hz Teensy torque loop remains stricter: two misses or 15 ms stops it. No new motor command is calculated from a missed sample. The UART parser discards a damaged late line and resynchronizes at the next checksum-valid reply. A lone missed active-error property reply is retried after 10 ms while normal feedback remains mandatory. UART ASCII is more interference-sensitive and timing-limited than CAN.
