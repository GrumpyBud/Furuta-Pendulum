...
- **A single impossible pendulum-overspeed spike:** compare adjacent CSV rows. The encoder estimator rejects a raw step above `100 rad/s` before it enters either velocity filter, and does not advance its reference on that rejected sample. Three consecutive rejected reads stop and invalidate zero; plausible filtered motion above `50 rad/s` still stops immediately. If repeated faults show multiple adjacent raw discontinuities, inspect magnet alignment, encoder mounting, supply decoupling, and SPI wiring rather than raising these limits.
