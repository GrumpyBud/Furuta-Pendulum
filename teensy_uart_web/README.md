...
- a separately gated guarded swing-up tuner that returns the arm to its saved center, waits for the pendulum to settle, then runs a 10 A-clamped, 20-second energy-shaping trial with automatic balance catch;
...
10. After bounded upright trials are consistently stable, use **Run** for guarded swing-up tuning. Let go with the arm stopped and the pendulum generally downward, then hold Space. Centering uses the tested ODrive settings: filtered input, `20 1/s` filter bandwidth, `0.5 turn/s` velocity limit, position gain `20`, velocity gain `0.167`, and velocity-integrator gain `0.333`, with a `0.300 N m` torque limit. The arm target must remain within `0.020 rad` and `0.050 rad/s`, while the hanging pendulum must remain within `0.040 rad` and `0.120 rad/s`, for 2.5 uninterrupted seconds. Only a verified torque-mode handoff can start the 20-second energy trial. At that handoff the firmware restores and verifies the separate `1.5 turn/s`, `0.500 N m per (turn/s)` torque-mode velocity envelope; this prevents the centering settings from silently reducing swing torque to `0.0835 N m`. The default swing law uses `1.0` strength, `0.030` damping, `0.180` centering, and a `0.160 N m` nudge. It preserves full pumping near center, progressively removes only outward energy torque from `0.65` to `1.10 rad`, and reserves increasing negative energy while approaching upright quickly. Swing torque can reverse at `30 N m/s` for timely braking; balance retains its validated `8 N m/s` slew limit. The web controls expose strength through `20.0`, damping and centering through `1.0`, startup nudge through `0.300 N m`, and a direct maximum-swing-torque setting through `0.450 N m`; the `0.4594 N m` (15 A-equivalent) firmware swing clamp cannot be bypassed. Balance remains limited to `0.3063 N m`, and upright tuning remains at `0.1225 N m`. At every swing arm request the firmware applies a session-only `15 A` ODrive motor soft limit, then reads back the motor soft limit, motor hard limit, and effective dynamic current limit; motion is refused unless the effective limit can deliver the requested ceiling. The reported `18 A` hard maximum remains the independent drive-side cutoff. The trial attempts the upright catch with the validated balance gains and logs all preparation/control modes and five swing settings in every CSV. Full automatic motion remains locked until these trials are reviewed.
...
- **UART faults during motion:** shorten/separate UART wiring, verify the ODrive is not emitting unrelated debug text, and inspect `loop_us`/`max_loop_us` in the downloaded CSV. Filtered position is sent once and retained inside ODrive; a short `u 0` feeds its watchdog without crowding the next feedback request. During ODrive-managed centering, arm feedback is requested at 100 Hz with a 9 ms reply window; a missed reply remains immediately due instead of advancing the schedule, and three consecutive misses or 35 ms without valid feedback stops the trial before the 50 ms ODrive watchdog expires. Active control exclusively owns feedback polling, so a tick between scheduled active requests cannot fall through to the shorter disarmed polling path. Successful arming and torque handoff restart the 200 ms property-health interval so their first active transaction is cyclic feedback. The 200 Hz Teensy torque loop remains stricter: two misses or 15 ms stops it. No new motor command is calculated from a missed sample. The UART parser discards a damaged late line and resynchronizes at the next checksum-valid reply. A lone missed active-error property reply is retried after 10 ms while normal feedback remains mandatory. UART ASCII is more interference-sensitive and timing-limited than CAN.
- **A single impossible pendulum-overspeed spike:** compare adjacent CSV rows. Because required swing speed is highest at hanging and lowest at upright, the raw estimator envelope varies from `400 rad/s` at hanging through `250 rad/s` at horizontal to `100 rad/s` at upright. It rejects only a step outside that angle-dependent envelope before either velocity filter and does not advance its reference. One isolated step is ignored; a second consecutive excessive step stops as pendulum overspeed while preserving zero. Actual SPI/parity failures remain a separate three-strike encoder fault that invalidates zero. Plausible filtered motion above `50 rad/s` still stops immediately. If repeated faults show multiple adjacent raw discontinuities, inspect magnet alignment, encoder mounting, supply decoupling, and SPI wiring rather than raising these limits.
...
- **Guarded swing-up will not start:** before holding Space on **Run**, stop the arm inside about ±100° of its saved center and let the pendulum be generally downward (within about 46°, below 2 rad/s). Once accepted, the ODrive centers the arm automatically. It faults if centering takes over 12 seconds, the strict arm/pendulum settling window is not held continuously for 2.5 seconds within the following 20 seconds, or the torque-mode handoff cannot be verified.
